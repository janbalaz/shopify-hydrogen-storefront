  ...
  switch (cartAction) {
    case CartAction.ADD_TO_CART:
      ...
      if (!cartId) {
        ...
        const chkt = await checkoutCreate({input, storefront});
        console.log(chkt);
      }
...
//! @see: https://shopify.dev/api/storefront/2022-01/mutations/checkoutcreate
...
export async function checkoutCreate({input, storefront}:{input: CheckoutCreateInput; storefront: AppLoadContext['storefront'];}) {
  const {checkoutCreate} = await storefront.mutate<{
    ...
    errors: UserError[];
  }>(CREATE_CHECKOUT_MUTATION, {variables: {input}});

  invariant(checkoutCreate, 'No data returned from checkoutCreate mutation');
